...
async function renameStage(projectId, stageId, name) { await update(ref(tenantDb, `projects/${projectId}/stages/${stageId}`), { name }); }
  ...
  return `
    <div class="flex flex-col gap-6 max-w-2xl">
      ...
      <div class="card p-5 flex flex-col gap-3">
        ...
        <p class="text-xs text-navy-600">Additional Master Admins are assigned by your platform Creator, not from here.</p>
      </div>
    </div>`;
}
...
function renderStageRow(projectId, stage, manage, canCheck) {
  ...
  return `
    <div class="stage-row ${stage.completed ? "completed" : ""} flex items-center gap-3 border border-navy-100 rounded-lg px-3 py-2.5">
      ...
      ${manage ? `
        <button data-action="edit-stage-name" data-project-id="${projectId}" data-stage-id="${stage.id}" data-current="${escapeHtml(stage.name)}" class="btn btn-ghost btn-sm !p-1.5" title="Rename stage" aria-label="Rename stage">
...
function openStageNamePromptModal(projectId, stageId, current) {
  openModal(`
    ${modalHeader("Rename Stage")}
    <form data-form="rename-stage" data-project-id="${projectId}" data-stage-id="${stageId}" class="p-5 flex flex-col gap-4">
      <div>
        <label class="field-label">Stage name</label>
        <input type="text" name="stageName" required value="${escapeHtml(current || "")}">
      </div>
      <div class="flex gap-2 justify-end mt-2">
        <button type="button" data-action="close-modal" class="btn btn-secondary">Cancel</button>
        <button type="submit" class="btn btn-primary">Save</button>
  ...
  try {
    switch (action) {
      ...
      case "edit-stage-name":
        openStageNamePromptModal(el.dataset.projectId, el.dataset.stageId, el.dataset.current);
        break;

      case "delete-stage":
        if (confirm("Delete this stage? This cannot be undone.")) {
          await deleteStage(el.dataset.projectId, el.dataset.stageId);
          if (state.projects[el.dataset.projectId]?.stages) {
            delete state.projects[el.dataset.projectId].stages[el.dataset.stageId];
          }
          toast("Stage deleted.");
      ...
      case "remove-assignee":
        ...
        if (state.projects[el.dataset.projectId]?.assignedEmails) {
          delete state.projects[el.dataset.projectId].assignedEmails[el.dataset.emailKey];
        }
      ...
      case "remove-team-member":
        ...
        if (state.teams[el.dataset.teamId]?.[el.dataset.field]) {
          delete state.teams[el.dataset.teamId][el.dataset.field][el.dataset.emailKey];
        }
      ...
      case "toggle-archive": {
        ...
        if (state.projects[el.dataset.id]) {
          state.projects[el.dataset.id].status = archived ? "active" : "archived";
        }
      ...
      case "delete-company":
        if (confirm("Remove this company from your registry? This only unlists it here — it does NOT delete the company's own database, teams, projects, or its Master Admin's ability to keep using it at the same URL. This cannot be undone from within Taskshi.")) {
          ...
          delete state.companies[el.dataset.id];
      ...
      case "delete-project":
        if (confirm("Permanently delete this project and all its stages? This cannot be undone.")) {
          ...
          delete state.projects[el.dataset.id];
...
document.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    else if (type === "rename-stage") {
      await renameStage(form.dataset.projectId, form.dataset.stageId, fd.get("stageName").trim());
      toast("Stage renamed."); openProjectModal(form.dataset.projectId);
    }
